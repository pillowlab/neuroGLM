function expt = addValue(expt, label, vardesc, associatedTiming)
% Indicate that the experiment has a value associated  observation
% expt = addValue(expt, label, vardesc)
%
% A non-timing value
%
%   label: 'string' - label for the spike train
%   vardesc: 'string' - longer description of where the spike train
%   associatedTiming: 'string' optional - label of the timing variable associated
%       with this value variable.
...
if nargin > 3
    if ~isfield(expt.meta.type, associatedTiming)
        error('Please add the associated timing variable first');
    end
